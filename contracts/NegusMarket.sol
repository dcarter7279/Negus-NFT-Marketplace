...
// Bring in the openzepplin ERC721 NFT functionality
...
contract NegusMarket is ReentrancyGuard {
	...
	/* number of items minting, number of transactions, tokens that have not been sold
	 ...
	 mapping(uint256 => MarketToken) private idToMarketTokenItem;
	...
	//2. create a market sale for buying ans selling between parties
	...
	idToMarketTokenItem[itemId] = MarketToken(
		itemId,
		nftContract,
		tokenId,
		payable(msg.sender),
	...
	function createMarketSale(
		...
		uint itemId)
		public payable nonReentrant {
			uint price = idToMarketTokenItem[itemId].price;
			uint tokenId = idToMarketTokenItem[itemId].tokenId;
			require(msg.value == price, 'Please submit the asking price in order to continue');

			// transfer the amount to the seller
			idToMarketTokenItem[itemId].seller.transfer(msg.value);
			// transfer the token from contract address to the buyer
			IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);
			idToMarketTokenItem[itemId].owner = payable(msg.sender);
			idToMarketTokenItem[itemId].sold = true;
	...
	function fetchMarketTokens() public view returns(MarketToken[] memory) {
		...
		for(uint i = 0; i < itemCount; i++) {
			if(idToMarketTokenItem[i + 1].owner == address(0)) {
				uint currentId = i + 1;
				MarketToken storage currentItem = idToMarketTokenItem[currentId];
	...
	function fetchMyNFTs() public view returns (MarketToken[] memory) {
		...
		// a seconf counts for each individual user
		uint itemCount = 0;
		uint currentIndex = 0;

		for(uint i = 0; i < totalItemCount; i++) {
			if(idToMarketTokenItem[i + 1].owner == msg.sender) {
				itemCount += 1;
			}
		}
		...
		for(uint i = 0; i < totalItemCount; i++) {
			if(idToMarketTokenItem[i + 1].owner == msg.sender) {
				uint currrentId = idToMarketTokenItem[i + 1].itemId;
				// current array
				MarketToken storage currentItem = idToMarketTokenItem[currrentId];
				items[currentIndex] = currentItem;
				currentIndex += 1;
			}
		}
	...
	for(uint i = 0; i < totalItemCount; i++) {
			if(idToMarketTokenItem[i + 1].seller == msg.sender) {
				itemCount += 1;
			}
		}
